...
def upload_image(request):
    if request.method == "POST" and request.FILES.get("image"):
        ...
        logger.info("Image uploaded: %s, session: %s", filename, request.session.session_key)
...
def studio(request):
    if "studio_image" not in request.session:
        logger.warning("No studio image in session, redirecting to index")
        return redirect("index")
    logger.info("Studio accessed, session image: %s", request.session.get("studio_image"))
...
def filter_image(request, tool):
    if request.method == "POST" and request.FILES.get("image"):
        image = request.FILES["image"]
        radius = float(request.POST.get("radius", 2.0)) if tool == "blur" else None
        intensity = float(request.POST.get("intensity", 0.5)) if tool in ["vignette", "noise"] else None
    ...
    if request.method == "POST" and request.FILES.get("image"):
        try:
            ...
            if tool == "super-resolution":
                ...
                points = [
                    (int(request.POST.get("x1", 0)), int(request.POST.get("y1", 0))),
                    (int(request.POST.get("x2", 0)), int(request.POST.get("y2", 0))),
                    (int(request.POST.get("x3", 0)), int(request.POST.get("y3", 0))),
                    (int(request.POST.get("x4", 0)), int(request.POST.get("y4", 0))),
                ]
                processor.perspective_correction(points)
            elif tool == "color-pop":
                hue_range = (int(request.POST.get("hue_min", 0)), int(request.POST.get("hue_max", 180)))
    ...
    if request.method == "POST" and request.FILES.get("image"):
        try:
            ...
            if tool == "convert-format":
                ...
                processed = processor.convert_format(format, quality, compression, width, height, aspect, strip, color, dpi, background)
